...
class Gallery extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
     ...
     */
    const GALLERY_TABLE = 'kuechenpate_brands_brand_media_gallery';

    const GALLERY_VALUE_TABLE = 'kuechenpate_brands_brand_media_gallery_value';

    const GALLERY_VALUE_TO_ENTITY_TABLE = 'kuechenpate_brands_brand_media_gallery_value_to_entity';
    /**#@-*/
    ...
    public function __construct(
        ...
        $this->metadata = $metadataPool->getMetadata(
            \Kuechenpate\Brands\Api\Data\BrandInterface::class
        );
     ...
     * @since 101.0.0
     */
    public function loadBrandGalleryByAttributeId($brand, $attributeId)
    {
        $select = $this->createBaseLoadSelect(
            $brand->getData($this->metadata->getLinkField()),
            $brand->getStoreId(),
            $attributeId
        );
     ...
     * @since 101.0.0
     */
    public function getBrandImages($brand, $storeIds)
    {
        if (!is_array($storeIds)) {
            $storeIds = [$storeIds];
        }

        $mainTable = $brand->getResource()->getAttribute('image')->getBackend()->getTable();
