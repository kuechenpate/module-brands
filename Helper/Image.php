...
class Image extends AbstractHelper implements ArgumentInterface
{
     ...
     * Current model
     *
     * @var \Kuechenpate\Brands\Model\Brand\Image
     */
    protected $_model;
     ...
     * @param \Kuechenpate\Brands\Model\Brand\ImageFactory $brandImageFactory
     * @param \Magento\Framework\View\Asset\Repository $assetRepo
     * @param \Magento\Framework\View\ConfigInterface $viewConfig
     * @param \Kuechenpate\Brands\Model\View\Asset\PlaceholderFactory $placeholderFactory
     * @param BrandMediaConfig $mediaConfig
     */
    public function __construct(
        \Magento\Framework\App\Helper\Context $context,
        \Kuechenpate\Brands\Model\Brand\ImageFactory $brandImageFactory,
        \Magento\Framework\View\Asset\Repository $assetRepo,
        \Magento\Framework\View\ConfigInterface $viewConfig,
        \Kuechenpate\Brands\Model\View\Asset\PlaceholderFactory $placeholderFactory = null,
        BrandMediaConfig $mediaConfig = null
    ) {
        $this->_brandImageFactory = $brandImageFactory;
        parent::__construct($context);
        $this->_assetRepo = $assetRepo;
        $this->viewConfig = $viewConfig;
        $this->viewAssetPlaceholderFactory = $placeholderFactory
            ?: ObjectManager::getInstance()->get(\Kuechenpate\Brands\Model\View\Asset\PlaceholderFactory::class);
        $this->mediaConfig = $mediaConfig ?: ObjectManager::getInstance()->get(BrandMediaConfig::class);
    }
     ...
     * @return $this
     */
    public function init($brand, $imageId, $attributes = [])
    {
        $this->_reset();

        $this->attributes = array_merge(
            $this->getConfigView()->getMediaAttributes('Kuechenpate_Brands', self::MEDIA_TYPE_CONFIG_NODE, $imageId),
            $attributes
        );
     ...
     *
     * @see \Kuechenpate\Brands\Model\Brand\Image
     * @param int $width
     * @param int $height
     * @return $this
     */
    ...
    {
        if ($placeholder) {
            $placeholderFullPath = 'Kuechenpate_Brands::images/brands/placeholder/' . $placeholder . '.jpg';
        } else {
            $placeholderFullPath = $this->_placeholder
                ?: 'Kuechenpate_Brands::images/brands/placeholder/' . $this->_getModel()->getDestinationSubdir() . '.jpg';
        }
        ...
        try {
            switch ($this->mediaConfig->getMediaUrlFormat()) {
                ...
                case BrandMediaConfig::HASH:
                    ...
                    break;
                default:
                    throw new LocalizedException(__("The specified Brand media URL format is not supported."));
            }
    ...
    /**
     * Return resized brand image information
     *
     * @return array
     */
     ...
     *
     * @return \Kuechenpate\Brands\Model\Brand\Image
     */
    protected function _getModel()
    {
        if (!$this->_model) {
            $this->_model = $this->_brandImageFactory->create();
        }
        return $this->_model;
    }
    ...
    public function getFrame()
    {
        ...
        if ($frame === null) {
            $frame = $this->getConfigView()->getVarValue('Kuechenpate_Brands', 'brand_image_white_borders');
        }
        return (bool)$frame;
    }
    ...
    {
        $label = $this->_brand->getData($this->getType() . '_' . 'label');
        if (empty($label)) {
            $label = $this->_brand->getName();
        }
        return $label;
    }
}
